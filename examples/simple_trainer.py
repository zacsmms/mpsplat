...
# `fused_ssim` is a CUDA-only extension. Fall back to torchmetrics' SSIM on
# MPS / CPU; the public signature is `fused_ssim(pred, target, padding=...)`.
try:
    from fused_ssim import fused_ssim
except ImportError:
    from torchmetrics.functional.image import structural_similarity_index_measure as _tm_ssim

    def fused_ssim(img1, img2, padding="valid"):  # noqa: ARG001 — keep signature
        return _tm_ssim(img1, img2, data_range=1.0, return_full_image=False)
...
from gsplat.cuda._backend import _sync as _sync_device
from gsplat.distributed import cli


def _max_mem_gb(device) -> float:
    """Best-effort current/peak allocator usage on `device`, in GiB.

    Returns 0 on backends without a usage query (CPU, current MPS releases).
    """
    ...
    if dev_type == "mps" and hasattr(torch.mps, "current_allocated_memory"):
...
@dataclass
class Config:
    # Override device (e.g. "mps", "cuda", "cpu"). Auto-detected if empty.
    device: str = ""
    # Disable viewer
...
class Runner:
    ...
    def __init__(
        ...
        from gsplat.cuda._dispatch import auto_device

        self.device = getattr(cfg, "device", None) or auto_device()
    ...
    def render_traj(self, step: int):
        ...
        if len(camtoworlds_all) < 2:
            print(
                f"[render_traj] only {len(camtoworlds_all)} keyframes available "
                "after trimming — skipping trajectory render"
            )
            return
