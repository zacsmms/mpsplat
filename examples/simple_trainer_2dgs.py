...
def _max_mem_gb_2dgs(device) -> float:
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.device = getattr(cfg, "device", "") or auto_device()
        self._sync_device = _sync_device
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.tb_every > 0 and step % cfg.tb_every == 0:
                mem = _max_mem_gb_2dgs(self.device)
    ...
    def render_traj(self, step: int):
        ...
        if len(camtoworlds) < 2:
            print(f"[render_traj] only {len(camtoworlds)} keyframes — skipping trajectory render")
            return
