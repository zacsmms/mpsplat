...
# Lidar paths are out of scope in the MPS port; keep a sentinel so any
# lidar-camera attempt fails loudly instead of silently calling a missing
# class (the original CUDA-era implementation lived in
# `gsplat.cuda._torch_lidars`, removed in Stage 0).
def _RowOffsetStructuredSpinningLidarModel(*_args, **_kwargs):
    raise NotImplementedError(
        "Lidar camera model is unsupported in the MPS port (out of scope)."
    )
# Lidar params come from a deleted module; alias to a sentinel class so that
# any caller passing `lidar_coeffs=<thing>` raises clearly when used with UT.
class RowOffsetStructuredSpinningLidarModelParametersExt:  # noqa: N801
    """Sentinel — lidar paths are out of scope in the MPS port."""
