...
# Distributed (NCCL-backed) helpers used to live in gsplat/distributed.py. The
# CUDA backend has been removed; multi-GPU paths are not reachable on MPS, so
# the four collectives below are stubbed out and only referenced from code
# paths gated on `torch.distributed.get_world_size() > 1` — which never fires
# on a single MPS device.
def _distributed_unsupported(*_args, **_kwargs):
    raise NotImplementedError(
        "Distributed collectives are unavailable in the MPS-only build. "
        "Run without initializing torch.distributed."
    )


all_gather_int32 = _distributed_unsupported
all_gather_tensor_list = _distributed_unsupported
all_to_all_int32 = _distributed_unsupported
all_to_all_tensor_list = _distributed_unsupported
...
def rasterization(
    ...
    packed: bool = False,  # MPS port: packed=True isect/rasterize wiring deferred
    tile_size: int = 16,
