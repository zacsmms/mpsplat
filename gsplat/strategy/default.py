...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if (
            ...
            if self.refine_scale2d_stop_iter > 0:
                state["radii"].zero_()
            from gsplat.cuda._backend import _empty_cache

            _empty_cache(state["grad2d"].device)
    ...
    def _update_state(
        ...
        if self.absgrad:
            absgrad = getattr(info[self.key_for_gradient], "absgrad", None)
            if absgrad is None:
                # MPS torch fallback hasn't populated `.absgrad` (the CUDA
                # path's autograd.Function did). Skip the running stats —
                # densification heuristics will run on the next step that
                # has gradients available.
                return
            grads = absgrad.clone()
        else:
            g = info[self.key_for_gradient].grad
            if g is None:
                return
            grads = g.clone()
